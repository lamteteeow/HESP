...
A GPU-accelerated 2D molecular dynamics / DEM simulator with **two-GPU domain decomposition**.

The simulation domain is split evenly along the X axis:
- **GPU 0** owns the left half `[xmin, split_x)`
- **GPU 1** owns the right half `[split_x, xmax)`

At each step:
1. **Halo exchange** — boundary particles are copied (via CPU) to the neighbor GPU as ghost/read-only particles, so cross-boundary contacts are captured.
...
5. **Particle migration** — particles that crossed split_x are redistributed.
6. **VTK output** — every `steps_per_frame` steps, both GPUs dump to a single VTK file.
...
| `Domain.h` | `Domain` struct + `buildDomains()` for 2-GPU X-split |
| `ParticleDevice.cuh` | GPU pointer struct (owned + halo layout) |
| `ParticleHost.h` | CPU buffer; `upload()` / `download()` / `freeParticleDevice()` |
| `halo_exchange.h` | `collectHalo()` / `uploadHalo()` / `exchangeHalos()` |
| `migration.h` | `migrateParticles()` — full CPU round-trip redistribution |
...
- **Capacity**: each GPU pre-allocates `2 * total_N` slots so the worst-case migration (all particles on one side) fits without reallocation.
...
- **Neighborhood table** (`d_nb0/d_nb1`): built once at startup per domain and never changes. Only `d_cellHeads` is reset each step.
- **Migration cost**: current implementation downloads all owned particles from both GPUs every time any particle crosses the boundary. This is the dominant cost for high-migration scenarios.
...
- [ ] **Extend to N GPUs**: generalize `buildDomains()` to split the X axis into N equal slices, one per GPU. Each interior GPU then has two halo neighbors (left and right); adjust `exchangeHalos()` accordingly.
