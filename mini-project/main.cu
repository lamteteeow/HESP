...
int main(const int argc, char** argv)
{
    ...
    char* endptr = nullptr;
    if (argc > 2) maxSteps = strtol(argv[2], &endptr, 10);
    if (endptr == argv[2])
    ...
    const Vec3 MAX_POS = {host.length.x + host.offset.x, host.length.y + host.offset.y, host.length.z + host.offset.z};
    ...
    cudaMemcpy(d_neighborsOfCell, h_neighborhood.data(), sizeForNumCellsINT * 27, cudaMemcpyHostToDevice);
    ...
    writeBoundingBoxWallsVTK(filename, maxSteps, "bounding_box.vtk", MIN_POS, MAX_POS);
    ...
    while (stepCount < maxSteps)
    {
        ...
        if (sd.n > 0)
        {
            assignCell<<<gridSpheres, block>>>(sd.n, sd.d_positions, NUM_CELLS, host.offset, cellL, sd.d_cellHeads, sd.d_cellTails, sd.d_cellIndexes);
        ...
        if (md.n > 0)
        {
            assignCell<<<gridComplexObj, block>>>(md.n, md.d_positions, NUM_CELLS, host.offset, cellL, md.d_cellHeads, md.d_cellTails, md.d_cellIndexes);
            err = cudaGetLastError();
            if (err != cudaSuccess)
            {
        ...
        if (sd.n > 0)
        {
            cfSphereOnSphere<<<gridSpheres, block>>>(host.gravity, sd, d_neighborsOfCell);
            err = cudaGetLastError();
            if (err != cudaSuccess)
            {
        ...
        if (md.n > 0)
        {
            cfMeshOnMesh<<<gridComplexObj, block>>>(host.gravity, md, d_neighborsOfCell);
            err = cudaGetLastError();
            if (err != cudaSuccess)
            {
        ...
        stepCount++;
        stepsToNextFrame--;
        if (stepsToNextFrame > 0) continue;
        ...
        //debug
        //printf("%d Total force: %f\n", stepCount, length(total));
        //VTK files
