...
inline void writeVTK(int frame,
                     const SphereVTK& vtk,
                     ...
                     long steps)
{
    ...
    {
        if (std::error_code error; !std::filesystem::create_directory(out_dir, error))
        {
            std::cerr << "Fehler: Konnte Verzeichnis " << out_dir << "nicht anlegen: " << error.message() << "\n";
            return;
        }
    }
    else if (!std::filesystem::is_directory(out_dir))
    {
        std::cerr << "Fehler: " << out_dir << " existiert, ist aber kein Verzeichnis.\n";
        return;
    }
    ...
    for (size_t i = 0; i < vtk.n; ++i) {
        vtk_file << vtk.positions[i].x << " " << vtk.positions[i].y << " " << vtk.positions[i].z << "\n";
    }
    ...
    for (size_t i = 0; i < vtk.n; ++i) {
        vtk_file << vtk.velocities[i].x << " " << vtk.velocities[i].y << " " << vtk.velocities[i].z << "\n";
    }
    ...
    vtk_file << "\nVECTORS force float\n";
    for (size_t i = 0; i < vtk.n; ++i) {
        vtk_file << vtk.forces[i].x << " " << vtk.forces[i].y << " " << vtk.forces[i].z << "\n";
    }
...
inline void writeMeshVTK(int frame,
                         const MeshVTK& vtk,
                         const std::string& postfix,
                         long steps)
{
    // same directory‐setup as writeSphereVTK…
    const std::string parent_prefix = "../";
    std::string cleaned_postfix = postfix;
    ...
    for (size_t i = 0; i < vtk.n; ++i) {
        ...
        int v1 = (mi+1<vtk.m ? vtk.meshes_offset[mi+1] : static_cast<int>(vtk.vertices_n));
        totalPts  += (v1 - v0);
        int f0 = vtk.faces_offset[mi];
        int f1 = (mi+1<vtk.m ? vtk.faces_offset[mi+1] : static_cast<int>(vtk.faces_n));
        totalCells += (f1 - f0);
    }
...
// Write bounding box as a separate clean VTK file
inline void writeBoundingBoxVTK(const std::string& filename, Vec3 minPos, Vec3 maxPos)
{
    std::ofstream vtk_file(filename);
    if (!vtk_file) throw std::runtime_error("Failed to open bounding box VTK file");

    vtk_file << "# vtk DataFile Version 3.0\n";
    vtk_file << "Bounding Box\n";
    vtk_file << "ASCII\n";
    ...
    vtk_file << "LINES 12 36\n";
    auto writeLine = [&](int a, int b) {
        vtk_file << "2 " << a << " " << b << "\n";
    };
    // Bottom
    writeLine(0, 1); writeLine(1, 2); writeLine(2, 3); writeLine(3, 0);
    // Top
    writeLine(4, 5); writeLine(5, 6); writeLine(6, 7); writeLine(7, 4);
    // Vertical
    writeLine(0, 4); writeLine(1, 5); writeLine(2, 6); writeLine(3, 7);

    vtk_file.close();
}
inline void writeBoundingBoxWallsVTK(const std::string& postfix,long steps,const std::string& filename, const Vec3 minPos, const Vec3 maxPos)
{
    const std::string parent_prefix = "../";
    std::string cleaned_postfix = postfix;
    while (cleaned_postfix.rfind(parent_prefix, 0) != std::string::npos)
