...
struct Quaternion
{
    ...
    __host__ __device__ Quaternion(const float w, const float x, const float y, const float z)
        : w(w), x(x), y(y), z(z)
    {
    }

    explicit __host__ __device__ Quaternion(const Vec3& vec) : w(1.0f), x(vec.x), y(vec.y), z(vec.z)
    {
    }

    __host__ __device__ __forceinline__ Quaternion operator+=(const Quaternion& q)
    {
        this->w += q.w;
        this->x += q.x;
        this->y += q.y;
    ...
    __host__ __device__ __forceinline__ Quaternion operator*(const Quaternion& q) const
    {
        return {
    ...
    __host__ __device__ __forceinline__ Quaternion operator*(const float s) const
    {
        return {w * s, x * s, y * s, z * s};
    }

    __host__ __device__ __forceinline__ friend Quaternion operator*(const float lhs, const Quaternion rhs)
    {
        return rhs * lhs;
    }
    ...
    __host__ __device__ __forceinline__ float3 toEulerAngles() const
    {
        ...
        return make_float3(roll * RAD2DEG,
                           pitch * RAD2DEG,
                           yaw * RAD2DEG);
    }
...
__host__ __forceinline__ Quaternion QuaternionFromJson(const nlohmann::json& a)
{
    return Quaternion{
        a[0].get<float>(),
        a[1].get<float>(),
