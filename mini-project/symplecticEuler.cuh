...
__device__ __forceinline__ void force(size_t i, size_t j, MeshDevice md,
                                      CollisionResult result, Vec3 &fi,
                                      Vec3 &ti) {
  ...
  // i,contactPointA.x,contactPointA.y,contactPointA.z,contactPointB.x,contactPointB.y,contactPointB.z,
  ...
  // printf("[%lu]rA:%f/%f/%f rB:%f/%f/%f velA:%f/%f/%f velB:%f/%f/%f
  // vrel:%f\n", i, rA.x, rA.y,
  // rA.z,rB.x,rB.y,rB.z,velocityA_p.x,velocityA_p.y,velocityA_p.z,
  // velocityB_p.x, velocityB_p.y, velocityB_p.z, v_rel_normal);
  // printf("[%lu]n:%f/%f/%f d:%f vrel:%f pos:%f/%f/%f\n", i, outNormal.x,
  // outNormal.y, outNormal.z, outDepth,v_rel_normal, md.d_positions[i].x,
  // md.d_positions[i].y, md.d_positions[i].z);
  ...
  // f_n_mag, f_t.x, f_t.y, f_t.z);

  ti += cross(rA, f_t);
}

__global__ inline void cfMeshOnMesh(const Vec3 grav, const MeshDevice md,
  ...
  for (int nbr = 0; nbr < 27; ++nbr) {
    ...
    for (int j = md.d_cellHeads[cell]; j != -1; j = md.d_cellTails[j]) {
      ...
      //printf("[%lu]cpA:%f/%f/%f cpB:%f/%f/%f cp:%f/%f/%f\n",
      i,contactPointA.x,contactPointA.y,contactPointA.z,contactPointB.x,contactPointB.y,contactPointB.z,
      contactPoint.x, contactPoint.y, contactPoint.z); Vec3 rA = contactPointA -
      pi; Vec3 rB = contactPointB - pj; Vec3 velocityA_p = vi +
      cross(md.d_angularVelocities[i], rA); Vec3 velocityB_p = vj +
      cross(md.d_angularVelocities[j], rB); float v_rel_normal = dot(velocityA_p
      - velocityB_p, outNormal);
      //printf("[%lu]rA:%f/%f/%f rB:%f/%f/%f velA:%f/%f/%f velB:%f/%f/%f
      vrel:%f\n", i, rA.x, rA.y,
      rA.z,rB.x,rB.y,rB.z,velocityA_p.x,velocityA_p.y,velocityA_p.z,
      velocityB_p.x, velocityB_p.y, velocityB_p.z, v_rel_normal);
      //printf("[%lu]n:%f/%f/%f d:%f vrel:%f pos:%f/%f/%f\n", i, outNormal.x,
      outNormal.y, outNormal.z, outDepth,v_rel_normal, md.d_positions[i].x,
      md.d_positions[i].y, md.d_positions[i].z); float f_n_mag = md.d_kn[i] *
      outDepth - md.d_gamma_n[i] * v_rel_normal; Vec3 f_n = outNormal * f_n_mag;
      fi+= f_n;
      ...
      if (v_surface_length > 1e-8f)
      {
          const float f_t_mag = fminf(md.d_gamma_t[i] * v_surface_length,
      coulomb); Vec3 v_hat_surface = -(v_surface / v_surface_length); f_t =
      v_hat_surface * f_t_mag; fi += f_t;
      }
      //printf("[%lu]fn:%f/%f/%f fnmag:%f ft:%f/%f/%f\n", i, f_n.x, f_n.y,
      f_n.z, f_n_mag, f_t.x, f_t.y, f_t.z);

      ti += cross(rA, f_t);*/

      force(i, j, md, result, out_fi, out_ti);
      // printf("[%lu]fi:%f/%f/%f out_fi:%f/%f/%f\n", i, fi.x, fi.y, fi.z,
      // out_fi.x, out_fi.y, out_fi.z); printf("[%lu]ti:%f/%f/%f
  ...
  // md.d_forces[i].y, md.d_forces[i].z, md.d_torques[i].x, md.d_torques[i].y,
  // md.d_torques[i].z);
}
...
__global__ inline void cfMeshOnPlane(const MeshDevice md,
  ...
  for (int p = 0; p < pd.n; ++p) {
    ...
    printf("[%lu]crossAngRa:%f/%f/%f ang:%f/%f/%f rA%f/%f/%f\n", i,
           crossAngRa.x, crossAngRa.y, crossAngRa.z, ang.x, ang.y, ang.z, rA.x,
           rA.y, rA.z);
    ...
    if (penetration > 1e-4f)
      fn_mag =
          fmaxf(md.d_kn[i] * penetration - md.d_gamma_n[i] * v_rel_n, 0.0f);
    else
      fn_mag = fmaxf(md.d_kn[i] * penetration,
                     0.0f); // keine Dämpfung bei sehr kleiner d
    ...
    ti += cross(rA, f_n);
    printf("[%lu]fn:%f/%f/%f fnmag:%f ft:%f/%f/%f\n", i, f_n.x, f_n.y, f_n.z,
           fn_mag, torque.x, torque.y, torque.z);
    ...
    if (v_tlen > 1e-8f) {
      float f_t_mag = fminf(md.d_gamma_t[i] * v_tlen, md.d_mu[i] * fn_mag);
...
__global__ inline void cfSphereOnSphere(const Vec3 grav, const SphereDevice sd,
                                        int *d_neighborsOfCell) {
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= sd.n)
    return;
  ...
  for (int k = 0; k < 27; ++k) {
    ...
    if (cellOrNeighbor == -1)
      continue;
    for (int j = sd.d_cellHeads[cellOrNeighbor]; j != -1;
         j = sd.d_cellTails[j]) {
      if (i == j)
        continue;
      ...
      const float f_n_mag =
          sd.d_kn[i] * overlap - sd.d_gamma_n[i] * v_rel_normal;
      ...
      if (v_surface_length > 1e-8f) {
        const float f_t_mag =
            fminf(sd.d_gamma_t[i] * v_surface_length, coulomb);
        Vec3 v_hat_surface = -(v_surface / v_surface_length);
        f_t = v_hat_surface * f_t_mag;
      }
...
__device__ inline void boundaryCollision(const int i, Vec3 *velocities,
                                         Vec3 *positions, const Vec3 MIN,
                                         const Vec3 MAX, const float *radii) {
