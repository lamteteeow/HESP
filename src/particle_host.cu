...
void ParticleHost::upload(ParticleDevice &pd, int total_cells, size_t total_n) {
  ...
  CHECK_CUDA(cudaMalloc(&pd.d_contact_count, sizeof(int)));
  CHECK_CUDA(cudaMalloc(&pd.d_mig_flag, sizeof(int)));
...
void ParticleHost::download(const ParticleDevice &pd) {
  n = pd.n;
  positions.resize(n);
  velocities.resize(n);
  masses.resize(n);
...
void freeParticleDevice(ParticleDevice &pd) {
  ...
  pd.d_ids = nullptr;
  CHECK_CUDA(cudaFree(pd.d_contact_count));
  pd.d_contact_count = nullptr;
  CHECK_CUDA(cudaFree(pd.d_mig_flag));
  pd.d_mig_flag = nullptr;
