...
void printStats(const std::chrono::duration<double> elapsedSeconds, 
				size_t nCells, 
				size_t nIt, 
    ...
    std::cout << "  bandwidth:     " 
    		  << 1e-9 * (numReads + numWrites) * sizeof(double) * nCells * nIt / elapsedSeconds.count() 
    		  << " GB/s\n";
}
